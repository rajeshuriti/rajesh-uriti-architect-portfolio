...
export function Navigation() {
  ...
  useMotionValueEvent(scrollY, 'change', (y) => {
    ...
    if (total > 0) {
      const idx = Math.min(Math.floor((y / total) * chapters.length), chapters.length - 1)
  ...
  return (
    <>
      ...
      >
        ...
        {/* Desktop: status chip */}
        <div className="hidden md:flex glass-card items-center gap-2 px-4 py-2 rounded-xl">
      ...
      <motion.div
        className="fixed right-4 lg:right-5 top-1/2 -translate-y-1/2 z-50 flex-col gap-2 hidden md:flex"
      ...
      >
        {chapters.map((ch, i) => (
          <button
            ...
            aria-label={ch.label}
          >
            <span className="opacity-0 group-hover:opacity-100 text-[10px] text-white/40 transition-opacity whitespace-nowrap">
              {ch.short}
            </span>
            <span
        ...
        {menuOpen && (
          ...
          >
            ...
            >
                  ...
                  >
                    <span
                      className={`w-2 h-2 rounded-full flex-shrink-0 transition-all duration-300 ${
                        i === activeChapter ? 'bg-[#00E5FF] scale-125' : 'bg-white/20'
                      }`}
                      style={
                        i === activeChapter
                    ...
                    <span
                      className={`text-sm font-medium transition-colors duration-200 ${
                        i === activeChapter ? 'text-[#00E5FF]' : 'text-white/60 group-hover:text-white'
                      }`}
                    >
                      {ch.label}
                    </span>
              ...
              {/* Footer */}
              <div className="flex items-center gap-2 mt-8 pt-6 border-t border-white/8">
