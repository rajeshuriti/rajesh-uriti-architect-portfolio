...
// SVG connection endpoints [x1%, y1%, x2%, y2%]
...
export function Chapter3Thinking() {
  ...
  const diagramScale   = useTransform(scrollYProgress, [0, 0.25], [0.88, 1])
  const diagramOpacity = useTransform(scrollYProgress, [0.72, 0.88], [1, 0])
    ...
    <article ref={containerRef} className="chapter-section" style={{ height: '300vh' }}>
      <div className="sticky-stage">
        <div className="relative z-10 w-full h-full flex flex-col items-center justify-center px-4">
          ...
          {/*
           * FIX: was a plain <div> with MotionValues in `style`.
           * MotionValues (opacity, scale) ONLY work on a <motion.*> element.
           * Mixing them with static CSS values (height) is fine on motion.div.
           */}
          <motion.div
          ...
          >
            <p className="text-xl sm:text-2xl lg:text-3xl font-display text-white leading-relaxed">
              &ldquo;The challenge was no longer{' '}
              <span className="gradient-heading font-bold">writing software.</span>
              <br />
              It was designing{' '}
              <span className="gradient-heading font-bold">systems capable of evolving.</span>&rdquo;
            </p>
            <div className="mt-6 flex flex-wrap gap-2 justify-center">
              {['API Design', 'System Design', 'Architecture Reviews', 'Domain-Driven Design', 'SOLID'].map((tag) => (
                <span
                  key={tag}
                  className="text-xs px-3 py-1 rounded-full border border-[#00E5FF]/20 text-[#00E5FF]/70"
                >
                  {tag}
                </span>
              ))}
